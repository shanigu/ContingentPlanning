...
namespace CPOR
{
    public class Action
    {
        ...
        {
            if(SDRPlanner.RemoveConflictingEffects)
            {
                if(f is CompoundFormula)
                {
                    CompoundFormula cf = (CompoundFormula)f;
                    CompoundFormula cfNew = cf.RemoveConflictingEffects();
                    f = cfNew;
                }
            }
            Effects = f;
        ...
        */
        ...
        private CompoundFormula CreateKnowledgeGainCondition(CompoundFormula cfCondition, List<string> lAlwaysKnown, bool bKnowWhether)
        {
            ...
            foreach (Predicate p in lEffects)
                ...
                {
                    if (bKnowWhether)
                        cfEffects.SimpleAddOperand(new KnowWhetherPredicate(p));
                    else
                    {
                        cfEffects.SimpleAddOperand(new KnowPredicate(p));
                        cfEffects.SimpleAddOperand(new KnowPredicate(p.Negate()).Negate());
                    }
                }
            if (cfEffects.Operands.Count == 0)
                return null;
            cfWhen.SimpleAddOperand(fPreconditions);
            cfWhen.SimpleAddOperand(cfEffects);
            return cfWhen;
        }
        //(f->g) ==> (Kf->Kg)
