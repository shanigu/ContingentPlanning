...
namespace CPOR
{
    class SDRPlanner
    {
        public static bool SDR_OBS = false;
        public static bool CPOR_OBS = true;
        ...
        public static bool RemoveConflictingEffects = true;
        public static bool UseManualSolve = false;
        public static bool RemoveUniversalQuantifiers = false;
        ...
        public void OfflinePlanning(string sPath, Domain domain, Problem problem)
        {
                    ...
                    else
                    {
                        ...
                        if (lPlan == null)
                        {
                            ...
                            */
                            ...
                            if (CPOR_OBS && pssCurrent.ExistsUncertainty())
                            {
                                List<Action> lObservationActions = bsInitial.Problem.Domain.GroundAllObservationActions(pssCurrent.Observed, pssCurrent.Hidden, true);
                                foreach (Action aObserve in lObservationActions)
                                {
                                    PartiallySpecifiedState psTrueState = null, psFalseState = null;
                                    ...
                                    if (psTrueState != null && psFalseState != null)
                                    {
                                        ...
                                        Console.WriteLine("CPOR_OBS: 2 " + aObserve.Name);
                                    }
                                    else if (psTrueState != null || psFalseState != null)
                                    {
                                        Console.WriteLine("CPOR_OBS: 1 " + aObserve.Name);
                                        if (psTrueState != null)
                                            pssCurrent = psTrueState;
                            ...
                            //Giving up on states that FF cannot solve
                            if (lPlan == null)
                            {
                                pssCurrent.UpdateClosedStates(lClosedStates, alreadyVisitedStates, domain);
                                pssCurrent = stateStack.Pop();
                ...
                if (lPlan != null)
                {
                    ...
                    { 
                        ...
                        if (!IsReasoningAction(sAction.ToLower()))
                        {
                            ...
                            Console.WriteLine("Applying " + sAction + " |H|=" + pssCurrent.m_lHistory.Count);
            ...
            WriteLocationTrajectoriesToFile(sPath, pssInitial);
        ...
        private void WriteTrajectoriesToFile(string sPath, PartiallySpecifiedState pssInitial)
        {
            StreamWriter sw = new StreamWriter(sPath + "/Trajectories.txt");
            List<List<string>> lTrajectotries = pssInitial.Plan.GetAllTrajectories();
            foreach(List<string> l in lTrajectotries)
            {
        ...
        private void WriteLocationTrajectoriesToFile(string sPath, PartiallySpecifiedState pssInitial)
        {
            StreamWriter sw = new StreamWriter(sPath + "/Trajectories.txt");
            List<List<string>> lTrajectotries = pssInitial.Plan.GetAllTrajectories();
            foreach (List<string> l in lTrajectotries)
            {
                ...
                foreach (string s in l)
                {
                    ...
                    {
                        string[] a = s.Replace("n", "").Split('_');
                        sLocation = "A:(" + a[1] + "," + a[2] + "," + a[3] + ")";
                    }
                    if (s.StartsWith("launchd"))
                    {
                        string[] a = s.Replace("n", "").Split('_');
                        sLocation = "D:(" + a[1] + "," + a[2] + "," + a[3] + ")";
                        iDX = int.Parse(a[1]);
                        iDY = int.Parse(a[2]);
                        iDZ = int.Parse(a[3]);
                    }
                        ...
                        if (!IsReasoningAction(sAction.ToLower()))
                        {
                            ...
                            else
                            {
                                ...
                                if (pssNext != null)
                                {
                                    ...
                                    if (SDR_OBS)
                                    {
                                        List<Action> lObservationActions = bsInitial.Problem.Domain.GroundAllObservationActions(pssNext.Observed, pssNext.Hidden, true);
