    ...
    public class BeliefState
    {
        ...
        */
        ...
        public State WriteTaggedDomainAndProblem(string sDomainFile, string sProblemFile, CompoundFormula cfGoal, List<Action> lAppliedActions, out int cTags, out MemoryStream msModels)
        {
            ...
            return WriteTaggedDomainAndProblem(sDomainFile, sProblemFile, cfGoal, lStates, DeadendStrategy.Lazy, out cTags, out msModels);
        }
        ...
        public State WriteTaggedDomainAndProblem(PartiallySpecifiedState pssCurrent, string sDomainFile, string sProblemFile, List<Action> lAppliedActions, out int cTags, out MemoryStream msModels)
        {
            ...
            if (SDRPlanner.WriteAllKVariations)
            {
                ...
                if (lStates.Count > 2)
                {
                    ...
                    for (int i = 0; i < lStates.Count - 1; i++)
                    {
                        ...
                        WriteTaggedDomainAndProblem(pssCurrent, sDomainFile.Replace(".pddl", cVersions + ".pddl"), sProblemFile.Replace(".pddl", cVersions + ".pddl"), lSelectedStates, DeadendStrategy.Lazy, out cTags, out msModels);
                        cVersions++;
                    }
                }
                else
                {
                    ...
                    WriteTaggedDomainAndProblem(pssCurrent, sDomainFile, sProblemFile, lStates, DeadendStrategy.Lazy, out cTags, out msModels);
                    cVersions = 1;
                }
                cTags = cVersions;
                return lStates[0];
            ...
            return WriteTaggedDomainAndProblem(pssCurrent, sDomainFile, sProblemFile, lStates, DeadendStrategy.Lazy, out cTags, out msModels);
        }
        ...
        public State WriteTaggedDomainAndProblemDeadEnd(PartiallySpecifiedState pssCurrent, string sDomainFile, string sProblemFile, List<Action> lAppliedActions, 
            List<Formula> lMaybeDeadends, DeadendStrategy dsStrategy, bool bPreconditionFailure, out int cTags, out MemoryStream msModels)
        {
            ...
            if (bPreconditionFailure)
                dsStrategy = DeadendStrategy.Active;

      
            return WriteTaggedDomainAndProblem(pssCurrent, sDomainFile, sProblemFile, lStates, dsStrategy, out cTags, out msModels);
        }
        public State WriteTaggedDomainAndProblem(string sDomainFile, string sProblemFile, CompoundFormula cfGoal, List<State> lStates, DeadendStrategy dsStrategy, out int cTags, out MemoryStream msModels)
        {
        ...
        public State WriteTaggedDomainAndProblem(PartiallySpecifiedState pssCurrent, string sDomainFile, string sProblemFile, List<State> lStates, DeadendStrategy dsStrategy, out int cTags, out MemoryStream msModels)
        {
            HashSet<Predicate> lObserved = new HashSet<Predicate>();
            Dictionary<string, List<Predicate>> dTags = GetTags(lStates, lObserved);
...
//#endif
        ...
        private List<List<Predicate>> ChooseDeadEndState(List<Formula> lMaybeDeadends, DeadendStrategy dsStrategy, bool bPreconditionFailure)
        {
        ...
        {
            if (SDRPlanner.DeadendStrategy == DeadendStrategy.Classical && m_lDeadendTags.Count > 0 && m_lProblematicTag == null)
            {
        ...
        public List<List<Predicate>> RunSatSolverMiniSat(List<Formula> lFormulas, int cAttempts, List<Predicate> lProblematicTag)
        {
            HashSet<Predicate> lPartialAssignment = new HashSet<Predicate>();
            if (!ApplyUnitPropogation(lFormulas, lPartialAssignment))
                return new List<List<Predicate>>();
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        foreach (string sVariable in m_sFFOutput.Split(' '))
                        {
                            ...
                            {
                                Predicate pAssigned = Problem.GetPredicateByIndex(idx - 1);
                                if (bNegate)
