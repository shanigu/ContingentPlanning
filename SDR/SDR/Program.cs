...
namespace PDDL
{
    class Program
    {
        //public static string BASE_PATH = @"C:\Users\lera\Dropbox\SDR\DeadEnds\Domains";
        public static string BASE_PATH = @"D:\Dropbox\SDR";
        ...
        static void TestKReplanner(string sBenchmarkPath, int cAttempts)
        {
            ...
            StaticBeliefState bsInitial = problem.GetInitialBelief();
        ...
        */
            ...
            void TestBenchmark(string sBenchmarkPath, string sBenchmark, int cTrials, bool bWriteResults)
            {
                ...
                //try
                {
                    ...
                    for (int i = 0; i < cTrials; i++)
                    {
                        ...
                        t.Name = "OnlineReplanningData " + domain.Name;
                        t.Start();
                        bool bFailed = false;
        ...
        static void TestWumpus(string sBenchmarkPath, int iSize)
        {
            ...
            //BeliefState.AddAllKnownToGiven = true;
            //TestCLG(sBenchmarkPath + sBenchmark + "\\", 2);
            TestBenchmark(sBenchmarkPath, sBenchmark, 25, false, false);
        }
        static void TestWumpusIII(string sBenchmarkPath, int iSize, int cPits, int cWumpuses)
        {
            SDRPlanner.AddAllKnownToGiven = true;
            //WumpusDomain mm = new WumpusDomain(iSize, 0, false, true);
            LargeWumpusDomainIII mm = new LargeWumpusDomainIII(iSize, cPits, cWumpuses);
            string sBenchmark = mm.Name;
            mm.WriteDomain(sBenchmarkPath + sBenchmark);
            mm.WriteProblem(sBenchmarkPath + sBenchmark);
            SDRPlanner.TagsCount = 2;
        ...
        static void TestBattleshipII(string sBenchmarkPath, int iSize)
        {
            SDRPlanner.AddAllKnownToGiven = true;
            BattleshipII bs = new BattleshipII(iSize);
            string sBenchmark = bs.Name;
            bs.WriteDomain(sBenchmarkPath + sBenchmark);
            bs.WriteProblem(sBenchmarkPath + sBenchmark);
            SDRPlanner.TagsCount = 2;
        ...
        {
            //ParseWidthSize(@"D:\Research\projects\PDDL\CLG_benchmarks\WidthAndSizeResults.cache");
            //ParseWidthSize(@"D:\Research\projects\PDDL\CLG_benchmarks\WidthAndSizeResults.nocache");
            ...
            string sBenchmarkPath = BASE_PATH + @"\PDDL\CLG_benchmarks\";
            Path = BASE_PATH + @"\PDDL\";
            ...
            /*
            MABlocksWorld2 w1 = null;
            w1 = new MABlocksWorld2(10, 6, 4);
            w1.WriteDomain(sBenchmarkPath + "/" + w1.Name);
            w1.WriteProblem(sBenchmarkPath + "/" + w1.Name);
            Parser p = new Parser();
            Domain d = p.ParseDomain(@"D:\Research\projects\PDDL\CLG_benchmarks\" + w1.Name + "\\d.pddl");
            Problem pr = p.ParseProblem(@"D:\Research\projects\PDDL\CLG_benchmarks\" + w1.Name + "\\p.pddl", d);

            GroundedPredicate.DEBUG = true;

            HashSet<Action> lActions = d.GroundAllActions(pr);
            Dictionary<Predicate, HashSet<Predicate>> lInvariants = d.IdentifyInvariants(lActions);
            ...
            */
            ...
            /*
            TestRockSample(sBenchmarkPath, 8, 4);
            TestRockSample(sBenchmarkPath, 8, 6);
            TestRockSample(sBenchmarkPath, 8, 8);
            TestRockSample(sBenchmarkPath, 8, 10);
            TestRockSample(sBenchmarkPath, 8, 12);
            
            TestWumpusIII(sBenchmarkPath, 10, 4, 4);
            TestWumpusIII(sBenchmarkPath, 10, 8, 8);
            TestWumpusIII(sBenchmarkPath, 10, 16, 16);

            TestBattleshipII(sBenchmarkPath, 6);
            TestBattleshipII(sBenchmarkPath, 8);
            TestBattleshipII(sBenchmarkPath, 10);
            TestBattleshipII(sBenchmarkPath, 12);
             */
            //TestBattleshipII(sBenchmarkPath, 12);
            ...
            string[] asBenchmarks = {"Wumpus05",  //"depot1", "depot10",
                                         //"zeno1", 
                                         //"zeno10", 
                                         //"satellite1", 
                                         //"satellite2"
            ...
            */
            ...
            TestAll(sBenchmarkPath, asBenchmarks, 25, false);
            //SDRPlanner.TagsCount = 6;
        ...
        //for Blai's MineSweeper
        static void MainBlai(string[] args)
        {
            ...
            string[] asMines = { //"mines-4x3",
                                   //"mines-4x4", 
                                   //"mines-8x8", 
                                   "mines-30x16" 
                               };

            foreach (string sMines in asMines)
            {
                string sBenchmarkPath = BASE_PATH + @"\PDDL\CLG_benchmarks\mines\" + sMines + "\\";
                SDRPlanner.SimulationStartState = null;
                for (int i = 0; i < 99; i++)
                {
                    string sModel = sMines + "-" + i;
                    string[] asBenchmarks = { };

                    SDRPlanner.SimulationStartState = new List<string>();
                    SDRPlanner.SimulationStartState.Add(@"D:\Research\projects\PDDL\CLG_benchmarks\mines\" + sMines + "\\" + sModel + "\\hidden");
                    SDRPlanner.GivenPlanFile = @"D:\Research\projects\PDDL\CLG_benchmarks\mines\" + sMines + "\\" + sModel + "\\plan";
                    SDRPlanner.Translation = SDRPlanner.Translations.SDR;

                    TestBenchmark(sBenchmarkPath, sModel, 1, true, false);

                    //TestAll(sBenchmarkPath, asBenchmarks, 3, false);
                }
            }
        }
        ...
        static void Main(string[] args)
        {
            Debug.Listeners.Add(new TextWriterTraceListener(Console.Out));
            Debug.Listeners.Add(new TextWriterTraceListener(new StreamWriter("debug.log")));
            string sBenchmarkPath = BASE_PATH + @"D:\Dropbox\SDR\Offline\CLG_benchmarks\Missles6\";
            Path = BASE_PATH + @"\PDDL\";
            SDRPlanner.AddTagRefutationToGoal = false;
            SDRPlanner.AddAllKnownToGiven = false;
            PartiallySpecifiedState.UseCFFBelief = false;
            ...
            if (args.Length > 0)
            {
                ...
                {
                    sPath = args[0];
                    sDomainFile = sPath + "/d.pddl";
                    sProblemFile = sPath + "/p.pddl";
                    sOutputFile = sPath + "/Output.pddl";
                }
                ...
                {
                    Console.WriteLine("Need the following parameters: DomainFileName ProblemFileName OutputFileName");
                    return;
                }
            }
            else
            {
                sPath = @"C:\Users\shanigu\OneDrive - Ben Gurion University of the Negev\Research\projects\SDR.1\Domains\wumpus05\";
                sDomainFile = sPath + "/d.pddl";
                sProblemFile = sPath + "/p.pddl";
                sOutputFile = sPath + "/Output.pddl";
            }

            if (sDomainFile.Contains("/"))
                sPath = sDomainFile.Substring(0, sDomainFile.LastIndexOf('/') + 1);
            else if (sDomainFile.Contains("\\"))
                sPath = sDomainFile.Substring(0, sDomainFile.LastIndexOf('\\') + 1);
            Parser p = new Parser();
            Domain d = p.ParseDomain(sDomainFile);
            Problem pr = p.ParseProblem(sProblemFile, d);

            SDRPlanner planner = new SDRPlanner(sPath, d, pr);
            int cActions, cObservations, cPlanning;
            TimeSpan ts;

            for (int iIteration = 0; iIteration < cIterations; iIteration++)
            {
                List<string> lSequence = planner.OnlineReplanning(sPath, d, pr, out cActions, out cPlanning, out cObservations, out ts);

                StreamWriter sw = new StreamWriter(sOutputFile, true);
                sw.WriteLine("Iteration " + iIteration);
                if (lSequence == null)
                    sw.WriteLine("Could not solve the problem");
                else
        ...
        static void MainMDP(string[] args)
        {
            Parser p = new Parser();
            //Domain d = p.ParseDomain(@"D:\Research\projects\PDDL\CLG_benchmarks\MDPs\domains\blocksworld\d.pddl");
            //Problem pr = p.ParseProblem(@"D:\Research\projects\PDDL\CLG_benchmarks\MDPs\domains\blocksworld\p.pddl", d);
            Domain d = null;
            Problem pr = null;
            p.ParseDomainAndProblem(@"D:\Research\projects\PDDL\CLG_benchmarks\MDPs\domains\blocksworld\p01-c0-C0-g1-n5.pddl", out  d, out pr);
            //p.ParseDomainAndProblem(@"D:\Research\projects\PDDL\CLG_benchmarks\MDPs\domains\boxworld\p01-b10-c5-dc0-fc0-dr0-gr1.pddl", out d, out pr);
            HashSet<Action> lActions = d.GroundAllActions(pr);
            //d.IdentifyMutexSAS(@"D:\Research\projects\PDDL\CLG_benchmarks\Elevators\", "d.pddl", "p2.pddl");
            //Domain d = p.ParseDomain(@"D:\Research\projects\PDDL\CLG_benchmarks\Logistics\d.pddl");
            //Problem pr = p.ParseProblem(@"D:\Research\projects\PDDL\CLG_benchmarks\Logistics\probLOGISTICS-4-0.pddl", d);
            StaticBeliefState bs = pr.GetInitialBelief();
            State s = bs.ChooseState(true);
            List<State> lStates = new List<State>();
            lStates.Add(s);
            for (int i = 0; i < 100; i++)
            {
                State sCurrent = lStates[i];
                HashSet<Action> lCurrentActions = d.GroundAllActions(sCurrent.Predicates, false);
                foreach (Action a in lCurrentActions)
                {
                    lStates.Add(sCurrent.Apply(a));
                }
            }
        ...
        static void ParseWidthSize(string sFileName)
        {
            StreamReader sr = new StreamReader(sFileName  + ".txt");
            Dictionary<string, Dictionary<int, int[]>> d = new Dictionary<string, Dictionary<int, int[]>>();
            while (!sr.EndOfStream)
            {
                string sLine = sr.ReadLine();
                string[] a = sLine.Split(',');
                string sName = a[0].Trim();
                int cSteps = int.Parse(a[1].Trim());
                int iSize = int.Parse(a[2].Trim());
                int iWidth = int.Parse(a[3].Trim());
                if (!d.ContainsKey(sName))
                    d[sName] = new Dictionary<int, int[]>();
                if (!d[sName].ContainsKey(cSteps))
                    d[sName][cSteps] = new int[5];
                d[sName][cSteps][0]++;
                d[sName][cSteps][1] += iSize;
                d[sName][cSteps][2] += iWidth;
                if (iSize > d[sName][cSteps][3])
                    d[sName][cSteps][3] = iSize;
                if (iWidth > d[sName][cSteps][4])
                    d[sName][cSteps][4] = iWidth;
            }
            sr.Close();

            StreamWriter sw = new StreamWriter(sFileName + ".parsed.txt");
            foreach (string sName in d.Keys)
            {
                foreach (int cSteps in d[sName].Keys)
                {
                    double dObservations = d[sName][cSteps][0];
                    sw.WriteLine(sName + "," + cSteps + "," + d[sName][cSteps][1] / dObservations + "," + d[sName][cSteps][2] / dObservations
                        + "," + d[sName][cSteps][3] + "," + d[sName][cSteps][4]
