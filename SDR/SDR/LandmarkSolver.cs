...
namespace PDDL
{
    class LandmarkSolver
    {
        ...
        public void IdentifyLandmarks(Problem p, Domain d)
        {
            ...
            HashSet<Action> lGrounded = d.GroundAllActions(p);
            Console.WriteLine("Removing disjunctions, conditional effects, and filtering impossible actions");
            HashSet<Action> lNoDisjunctions = RemoveDisjunctions(lGrounded);//remove disjunctions before grounding?
            HashSet<Action> lSimple = RemoveConditions(lNoDisjunctions);
            ...
            Dictionary<Predicate, HashSet<Action>> dPreconditionToAction = new Dictionary<Predicate, HashSet<Action>>();
            Dictionary<Predicate, HashSet<Action>> dEffectToAction = new Dictionary<Predicate, HashSet<Action>>();
            ...
            while (qOpenLandmarks.Count > 0)
            {
                ...
                HashSet<Action> lFiltered = new HashSet<Action>(), lPotentialLandmarkAchievers = new HashSet<Action>();
                ...
                Dictionary<Predicate, HashSet<Action>> dLandmarkAchievers = GetLandmarkAchievers(fCurrent, lAllReachable, lPotentialLandmarkAchievers);
        ...
        private HashSet<Action> RemoveNegativePreconditions(HashSet<Action> lActions)
        {
            HashSet<Action> lDeleteRelaxation = new HashSet<Action>();
        ...
        private HashSet<Action> FilterImpossible(HashSet<Action> lActions, HashSet<Predicate> lInitialState, Domain d)
        {
            HashSet<Action> lPossible = new HashSet<Action>();
            HashSet<Action> lImpossible = new HashSet<Action>();
            foreach (Action a in lActions)
            {
                bool bPossible = true;
        ...
        private void ClassifyActions(HashSet<Action> lSimple, Dictionary<Predicate, HashSet<Action>> dPreconditionToAction, Dictionary<Predicate, HashSet<Action>> dEffectToAction)
        {
            foreach (Action a in lSimple)
            {
                ...
                foreach (Predicate p in lPre)
                {
                    if (!dPreconditionToAction.ContainsKey(p))
                        dPreconditionToAction[p] = new HashSet<Action>();
                ...
                foreach (Predicate p in lEffects)
                {
                    if (!dEffectToAction.ContainsKey(p))
                        dEffectToAction[p] = new HashSet<Action>();
        ...
        private HashSet<Action> RemoveConditions(HashSet<Action> lActions)
        {
            HashSet<Action> lNoConditions = new HashSet<Action>();
            foreach (Action a in lActions)
            {
        ...
        private HashSet<Action> RemoveDisjunctions(HashSet<Action> lGrounded)
        {
            HashSet<Action> lNoDisjunctions = new HashSet<Action>();
        ...
        private void RemoveDisjunctions(Action a, List<List<Formula>> lOptions, int iOption, CompoundFormula cfAnd, HashSet<Action> lNoDisjunctions, ref int iIndex)
        {
        ...
        private HashSet<Predicate> GetJointPreconditions(HashSet<Action> lActions)
        {
        ...
        private Dictionary<string, Dictionary<string, List<Predicate>>> GetDisjunctivePreconditions(HashSet<Action> lActions, HashSet<Predicate> lInitialState)
        {
...
/*
        private List<Formula> GetLandmarks(Formula fCurrentLandmark, Dictionary<Predicate, HashSet<Action>> dLandmarkAchievers, HashSet<Predicate> lInitialState)
        {
            ...
            {
                HashSet<Action> lActions = dLandmarkAchievers[pKey];
                if (lActions.Count == 0)
                    continue;
...
*/
        ...
        private bool RegressLandmark(Predicate pCurrentLandmark, HashSet<Action> lLandmarkAchievers, HashSet<Predicate> lInitialState,
        ...
        private List<Formula> RegressLandmark(Formula fCurrentLandmark, Dictionary<Predicate, HashSet<Action>> dLandmarkAchievers, HashSet<Predicate> lInitialState)
        {
            ...
            {
                HashSet<Action> lActions = dLandmarkAchievers[pLandmark];
        ...
        private Dictionary<Predicate, HashSet<Action>> GetLandmarkAchievers(Formula fLandmark, HashSet<Predicate> lAllReachable, HashSet<Action> lPotentialLandmarkAchievers)
        {
            Dictionary<Predicate, HashSet<Action>> dActions = new Dictionary<Predicate, HashSet<Action>>();
            HashSet<Predicate> lLandmark = fLandmark.GetAllPredicates();
            foreach (Predicate p in lLandmark)
            {
                dActions[p] = new HashSet<Action>();
        ...
        private HashSet<Predicate> ReachablePredicates(HashSet<Predicate> lInitial, HashSet<Action> lFiltered)
        {
            HashSet<Predicate> lReachable = new HashSet<Predicate>(lInitial);
            HashSet<Action> lUnapplied = new HashSet<Action>(lFiltered);
            bool bChanged = true;
            while (bChanged)
            {
                bChanged = false;
                HashSet<Action> lNewUnapplied = new HashSet<Action>();
        ...
        private void FilterActions(HashSet<Action> lGrounded, Formula fCurrent, HashSet<Action> lFiltered, HashSet<Action> lLandmarkAchievers)
        {
        ...
        private void FilterActions(HashSet<Action> lGrounded, Formula fCurrent, HashSet<Action> lFiltered, HashSet<Action> lLandmarkAchievers, 
            Dictionary<Predicate, HashSet<Action>> dPreconditionToAction, Dictionary<Predicate, HashSet<Action>> dEffectToAction, 
        ...
        public List<Action> SolveII(Problem p, Domain d)
        {
            ...
            while (lOpenList.Count > 0)
            {
                ...
                HashSet<Action> lActions = d.GroundAllActions(sCurrent.Predicates, false);
                foreach (Action a in lActions)
                {
        ...
        {
            State sStart = p.GetInitialBelief().ChooseState(true);
            List<Action> lActions = new List<Action>(d.GroundAllActions(sStart.Predicates, false));
        ...
        public HashSet<Action> Solve(Problem p, Domain d)
        {
            State sStart = p.GetInitialBelief().ChooseState(true);
            HashSet<Action> lActions = new HashSet<Action>();
        ...
        public HashSet<Action> SolveOld(Problem p, Domain d)
        {
            State sStart = p.GetInitialBelief().ChooseState(true);
            HashSet<Action> lActions = new HashSet<Action>();
        ...
        private State CloseAll(State s, HashSet<Action> lActions, Domain d)
        {
            State sCurrent = s;
            HashSet<Action> l = d.GroundAllActions(s.Predicates, false);
        ...
        private State ApplyCompute(State s, string sName, HashSet<Action> lActions, Domain d)
        {
            State sCurrent = s;
        ...
        private State ApplyAxiom(State s, HashSet<Action> lActions, Domain d)
        {
            State sCurrent = s;
        ...
        private State ObserveAll(State s, HashSet<Action> lActions, Domain d)
        {
            State sCurrent = s;
            HashSet<Action> l = d.GroundAllActions(s.Predicates, false);
            foreach (Action a in l)
