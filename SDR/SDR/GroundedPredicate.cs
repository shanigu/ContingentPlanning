...
namespace PDDL
{
    public class GroundedPredicate : Predicate
    {
        ...
        public void AddConstant(Constant c)
        {
            ...
            if (DEBUG)
            {
                if (Name == "in-stack" && Constants.Count == 2 && Constants[0].Name.StartsWith("T"))
                {
                    int i = int.Parse(Constants[0].Name.Substring(1));
                    int j = int.Parse(Constants[1].Name.Substring(1));
                    if (i != j)
                        Console.WriteLine("*");
                }
            }
