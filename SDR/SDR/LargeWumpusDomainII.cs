...
namespace PDDL
{
    class LargeWumpusDomainII
    {
        public static int WumpusCount { get; set; }
        public static int PitCount { get; set; }
        public static int Size { get; private set; }
        public LargeWumpusDomainII(int cSquares, int cPits, int cWumpuses)
        {
        ...
        private string GetSafes()
        {
            ...
            for (int x = 1; x <= Size; x++)
            {
                ...
                {
                    sSafe += "(unknown (safe " + GetPosition(x, y) + "))\n";
                    sSafe += "(unknown (wp-at " + GetPosition(x, y) + "))\n";
                }
            }
            return sSafe;
        }
        ...
        private string GenerateInit1Action()
        {
            ...
            for (int x = 1; x <= Size; x++)
            {
                for (int y = 1; y <= Size; y++)
                {
                    ...
                    sAction += "\t(when (and (not (safe " + sPosition + ")) (wp-at " + sPosition + ")) (wumpus-at " + sPosition + "))\n";
                    sAction += "\t(when (and (not (safe " + sPosition + ")) (not (wp-at " + sPosition + "))) (pit-at " + sPosition + "))\n";
                }
            }
            sAction += "))\n";
            return sAction;
                ...
                for (int y = 1; y <= Size; y++)
                {
                    ...
                    for (int i = -1; i <= 1; i++)
                    {
                        for (int j = -1; j <= 1; j++)
                        {
                            ...
                            {
                                sStenchPosition = GetPosition(x + i, y + j);
        ...
        public string Name { get { return "LargeWumpusII-" + Size; } }
