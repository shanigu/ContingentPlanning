...
namespace PDDL
{
    public class StaticBeliefState
    {
        public IEnumerable<Predicate> Observed { get { return m_lObserved; } }
        public IEnumerable<CompoundFormula> Hidden { get { return m_lHiddenFormulas; } }
        ...
        private StaticBeliefState m_sPredecessor;
        ...
        public StaticBeliefState(Problem p)
        {
            Problem = p;
            m_sPredecessor = null;
        ...
        public StaticBeliefState(StaticBeliefState sPredecessor)
            : this(sPredecessor.Problem)
        {
            m_sPredecessor = sPredecessor;
            m_lHiddenFormulas = new List<CompoundFormula>(sPredecessor.Hidden);
            m_lOriginalHiddenFormulas = new List<CompoundFormula>(sPredecessor.m_lOriginalHiddenFormulas);
            m_dMapPredicatesToFormulas = new Dictionary<GroundedPredicate, List<int>>(sPredecessor.m_dMapPredicatesToFormulas);
            m_lObserved = new HashSet<Predicate>(sPredecessor.Observed);
            m_cfCNFHiddenState = sPredecessor.m_cfCNFHiddenState;
        }
        ...
        public bool ConsistentWith(Formula f)
        {
            ...
            else
            {
                ...
                CompoundFormula cfCNF = cf.ToCNF();
        ...
        {
            if (obj is StaticBeliefState)
            {
                StaticBeliefState bs = (StaticBeliefState)obj;
        ...
        public virtual StaticBeliefState Clone()
        {
            return new StaticBeliefState(this);
        }
        public static TimeSpan tsTotalTime = new TimeSpan();

        //special case - (p v q) and (p v ~q) => p
        private Predicate CheckSimpleReasoning(CompoundFormula cf)
        {
            ...
            {
                if (m_lHiddenFormulas[idx] != null && m_lHiddenFormulas[idx].Operands.Count == 2)
                {
                    HashSet<Predicate> lOtherPredicates = m_lHiddenFormulas[idx].GetAllPredicates();
                    if (lOtherPredicates.Contains(lPredicates[0]) && lOtherPredicates.Contains(lPredicates[1].Negate()))
                        return lPredicates[0];
                    if (lOtherPredicates.Contains(lPredicates[0].Negate()) && lOtherPredicates.Contains(lPredicates[1]))
                        return lPredicates[1];
                }
            }
            return null;
        }
        ...
        public bool AddReasoningFormula(Formula f)
        {
            ...
            if (f is CompoundFormula)
            {
                ...
                if (cf.Operator == "and")
                {
                    foreach (Formula fSub in cf.Operands)
                    {
                        ...
                        {
                            Predicate p = CheckSimpleReasoning((CompoundFormula)fSub);
                            if (p != null)
                            {
            ...
            return bLearnedNewPredicate;
        }




        public bool AddReasoningFormulaEfficient(Formula f)
        {
            DateTime dtStart = DateTime.Now;
        ...
        */
            ...
            else if (SDRPlanner.SimulationStartState == null || UnderlyingEnvironmentState != null)
            {
                ...
                else
                {
                    ...
                    Debug.Write("Choosing hidden variables ");
                        ...
                        //Debug.Write(".");
                    lAssignment = ChooseHiddenPredicates(m_lHiddenFormulas, false);
                    if (lAssignment == null)
                        return null;
                    //Debug.WriteLine("");
        ...
        private string GetRandomUniquePosition(List<string> lPositions, int iSize)
        {
            while (true)
            {
        ...
        private State ChooseStateForLargeDomains()
        {
            ...
            {
                int iSize = LargeWumpusDomain.Size;
                int cWumpuses = LargeWumpusDomain.WumpusCount, cPits = LargeWumpusDomain.PitCount;
                if (Problem.Name.Contains("II"))
                {
                    iSize = LargeWumpusDomainIII.Size;
                    cWumpuses = LargeWumpusDomainIII.WumpusCount;
                    cPits = LargeWumpusDomainIII.PitCount;
                }
                List<string> lPositions = new List<string>();
                string posGold = GetRandomUniquePosition(lPositions, iSize);
                int xGold = int.Parse(posGold.Split(',')[0]);
                int yGold = int.Parse(posGold.Split(',')[1]);
                /*
                ...
                for (int i = 1; i <= yGold; i++)
                {
                    lPositions.Add((xGold + 1) + "," + i);
                    lPositions.Add(xGold + "," + i);
                    lPositions.Add((xGold - 1) + "," + i);
                }
                  */  
                ...
                for (int i = 0; i < cPits; i++)
                {
                    ...
                    if (Problem.Name.Contains("II"))
                    {
                        gpPit = new GroundedPredicate("wp-at");
                        gpPit = (GroundedPredicate)gpPit.Negate();
                    }
                ...
                for (int iX = 1; iX <= iSize; iX++)
                    for (int iY = 1; iY <= iSize; iY++)
                    {
                        ...
                        if (lPositions.Contains(iX + "," + iY) && (iX != xGold || iY != yGold))
                            s.AddPredicate(gpSafe.Negate());
                        else
            ...
            {
                int iSize = Battleship.Size;
                int cShipTypes = Battleship.ShipTypes;
                int[] aShipCount = Battleship.ShipCount;
                if (Problem.Name.Contains("II"))
                {
                    iSize = BattleshipII.Size;
                    cShipTypes = BattleshipII.ShipTypes;
                    aShipCount = BattleshipII.ShipCount;
                }


                int[,] aMap = new int[iSize, iSize];
                List<GroundedPredicate> lPredicates = new List<GroundedPredicate>();

                int cFailures = 0;
                for (int iShipType = cShipTypes ; iShipType > 0; iShipType--)
                {
                    int cShips = aShipCount[iShipType];
                    ...
                    for (int iShip = 0; iShip < cShips; iShip++)
                    {
                        ...
                        {
                            cAttempts--;
                            int iX = RandomGenerator.Next(iSize - iLength);
                            int iY = RandomGenerator.Next(iSize - iLength);
                            ...
                            if (bVertical)
                            {
                                ...
                                if (bFree)
                                {
                                    ...
                                    {
                                        for (int j = -1; j <= 1; j++)
                                            if (iX + j >= 0 && iX + j < iSize && iY + i >= 0 && iY + i <iSize)
                                            {
                                                if(aMap[iX + j, iY + i] == 0)
                                                    aMap[iX + j, iY + i] = 1;
                                            }
                            ...
                            else
                            {
                                ...
                                if (bFree)
                                {
                                    ...
                                    {
                                        for (int j = -1; j <= 1; j++)
                                            if (iX + i >= 0 && iX + i < iSize && iY + j >= 0 && iY + j < iSize)
                                            {
                                                if(aMap[iX + i, iY + j] == 0)
                                                    aMap[iX + i, iY + j] = 1;
            ...
            foreach (CompoundFormula cf in lHidden)
            {
                if (cf != null)
                {
                    ...
                    if (fReduced is PredicateFormula)
                    {
                        ...
                        if (pReasoned.Name != Domain.TRUE_PREDICATE)
                        {
        ...
        private List<Predicate> ChooseHiddenPredicates(List<CompoundFormula> lHidden, List<Predicate> lAssignment, List<Predicate> lUnknown, List<List<Predicate>> lCurrentAssignments)
        {
            ...
            if (bAllTrue)
                pCurrent = pCurrent.Negate();
            else if (!bAllFalse && RandomGenerator.NextDouble() < 0.5)
            {
                pCurrent = pCurrent.Negate();
            }
        ...
        /*
        public State WriteTaggedDomainAndProblem(string sDomainFile, string sProblemFile)
        {
            List<List<Predicate>> lStates = ChooseStateSet();
            Dictionary<string, List<Predicate>> dTags = GetTags(lStates);
            if (OutputType == "PDDL")
            {
                throw new NotImplementedException();
                Problem.Domain.WriteTaggedDomain( dTags, Problem);
                //WriteTaggedProblem(sProblemFile, dTags, m_lObserved, dTags.Values.First(), false); //the first tag is the real state
            }
            else
            {

            }
            State sChosen = new State(Problem);
            foreach (GroundedPredicate p in Observed)
                sChosen.AddPredicate(p);
            foreach (GroundedPredicate p in dTags.Values.First())
                sChosen.AddPredicate(p);
            return sChosen;
        }
        */
        ...
        public State WriteTaggedDomainAndProblem(string sDomainFile, string sProblemFile, List<Action> lAppliedActions, out int cTags, out MemoryStream msModels)
        {
            ...
            if (SDRPlanner.WriteAllKVariations)
            {
                ...
                if (lStates.Count > 2)
                {
                    ...
                    for (int i = 0; i < lStates.Count - 1; i++)
                    {
                        ...
                        WriteTaggedDomainAndProblem(sDomainFile.Replace(".pddl", cVersions + ".pddl"), sProblemFile.Replace(".pddl", cVersions + ".pddl"), lSelectedStates, false, out cTags, out msModels);
                        cVersions++;
                    }
                }
                else
                {
                    ...
                    WriteTaggedDomainAndProblem(sDomainFile, sProblemFile, lStates, false, out cTags, out msModels);
                    cVersions = 1;
                }
                cTags = cVersions;
                return lStates[0];
            ...
            return WriteTaggedDomainAndProblem(sDomainFile, sProblemFile, lStates, false, out cTags, out msModels);
        }
        public State WriteTaggedDomainAndProblem(string sDomainFile, string sProblemFile, List<State> lStates, bool bOnlyIdentifyStates, out int cTags, out MemoryStream msModels)
        {
        ...
        private Dictionary<string, List<Predicate>> GetTags(List<State> lStates, List<Predicate> lObserved)
        {
        ...
        private List<List<Predicate>> ChooseStateSet()
        {
            ...
            //TODO:DORIN - if there are any possible deadends, choose one state which is a deadend, and one state which is not a deadend (should be the "real" state) and plan to solve the non-deadend state
            //if there are no deadends, use the code below (revise existing tags)
        ...
        private List<List<Predicate>> ChooseDiverseStateSet(int cStates, List<Predicate> lCurrentTag)
        {
            ...
            {
                List<Predicate> lAssignment = ChooseHiddenPredicates(lConstraints, lStates, false);
        ...
        public bool ApplyUnitPropogation(List<Formula> lFormulas, List<Predicate> lAssignment)
        {
            DateTime dtStart = DateTime.Now;
            List<PredicateFormula> lLearnedPredicates = new List<PredicateFormula>();
        ...
        {
            List<Predicate> lPartialAssignment = new List<Predicate>();
            ...
            {
                lAssignments.Add(lPartialAssignment);
                return lAssignments;
            }
            ...
            while (cAttempts > 0)
            {
                ...
                //p.StartInfo.FileName = @"C:\Users\User\Dropbox\SDR\Offline\Planners\MiniSat.exe";
                p.StartInfo.FileName = Program.BASE_PATH + @"\PDDL\Planners\" + "MiniSat.exe";
                ...
                {
                    Console.WriteLine("BUGBUG SatSolver");
                }
            }
...
*/
        ...
        int cRegressions = 0;
        public Dictionary<int, int[]> MaxWidthSize = new Dictionary<int, int[]>();

        public bool ReviseInitialBelief(Formula fObserve, PartiallySpecifiedState pssLast)
        {
            ...
            while (pssCurrent.Predecessor != null)
            {
                ...
                {
                    if (fCurrent.IsTrue(pssCurrent.Known))
                    ...
                    //if (fObserve.ToString() == "(not (stench p-2 p-9))" && pssCurrent.GeneratingAction.Name == "init2")
                    //    Console.Write("*");
                ...
                if (SDRPlanner.UseCaching && hsNew.Count > 0 && pssSuccessor != null)
                ...
                foreach (Formula f in lRegressedFormulas)
                {
                    HashSet<Predicate> h = f.GetAllPredicates();
                    foreach (Predicate p in h)
                        hsAll.Add(p.Canonical());
                    iSumSize += f.Size;
                }
                if (iMaxSize < iSumSize)
                    iMaxSize = iSumSize;
                if (iMaxWidth < hsAll.Count)
                    iMaxWidth = hsAll.Count;

            }
            MaxWidthSize[cRegressions] = new int[] { iMaxSize, iMaxWidth };
            ...
            if (lCurrentFormulas.Count == 1)
            {
                ...
                if (fFinal is CompoundFormula)
                {
                    fFinal = ((CompoundFormula)fFinal).ToCNF();
                }
            }
            else
            {
                ...
                foreach (Formula f in lCurrentFormulas)
                    cf.AddOperand(f.Reduce(Observed));
                fFinal = cf.ToCNF();
            }
...
//            if (count_revisions == 65)
//                Console.Write("*");

            bool bUpdate = true;
            if (StaticBeliefState.UseEfficientFormulas)
                bUpdate = AddReasoningFormulaEfficient(fFinal);
            else
                bUpdate = AddReasoningFormula(fFinal);

            if (bUpdate)
            {
                HashSet<Predicate> lLearned = pssCurrent.ApplyReasoning();
                dtAfterReasoning = DateTime.Now;
                if (SDRPlanner.UseCaching)
                {
                    if (bTrueRegression)
                    {
                        //while (bUpdate && sTrace.Count > 0)
                        while (sTrace.Count > 0 && lLearned != null && lLearned.Count > 0)
                        {
        ...
        public FactoredBeliefState GetFactoredBelief()
        {
            Domain d = Problem.Domain.Ground(Problem);
            FactoredBeliefState fbs = new FactoredBeliefState(d, Problem);
            fbs.UnderlyingEnvironmentState = UnderlyingEnvironmentState;
