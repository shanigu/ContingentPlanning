...
namespace PDDL
{
    public class Action
    {
        ...
        {
            //if (sName.Contains("activate-bandagebag0-sensor-at-p4-1"))
            //    Console.Write("*");
            Name = sName;
        ...
        public void RemoveDisjunctiveEffects()
        {
            ...
            foreach (Predicate p in lMandatory)
                cfNewEffects.SimpleAddOperand(p);
            foreach(CompoundFormula cfCondition in lConditions)
            {
                if (cfCondition.Operands[0] is CompoundFormula)
                {
                    CompoundFormula cfGiven = (CompoundFormula)cfCondition.Operands[0];
                    if(cfGiven.Operator == "or")
                    {
                        foreach(Formula fSub in cfGiven.Operands)
                        {
                            CompoundFormula cfNewCondition = new CompoundFormula("when");
                            cfNewCondition.AddOperand(fSub);
        ...
        */
        ...
        private CompoundFormula CreateTaggedKnowledgeGainCondition(CompoundFormula cfCondition, string sTag, List<string> lAlwaysKnown, bool bNonDetEffect)
        {
            ...
            foreach (Predicate p in lEffects)
            {
                ...
                {
                    Predicate pKEffect = p.GenerateKnowGiven(sTag);
                    Predicate pKNegateEffect = p.Negate().GenerateKnowGiven(sTag).Negate();
                    cfEffects.AddOperand(pKEffect);
                    cfEffects.AddOperand(pKNegateEffect);
                    if (bNonDetEffect)
                    {
        ...
        public void SimplifyConditions()
        {
            ...
            if (Effects != null)
            {
                ...
                foreach (CompoundFormula cfCondition in lConditions)
                {
                    ...
                    {
                        CompoundFormula cfNewCondition = new CompoundFormula("when");
                        CompoundFormula cfNewFirst = ((CompoundFormula)cfCondition.Operands[0]).ToCNF();
