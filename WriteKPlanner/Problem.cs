...
namespace ContingentPlanning
{
    public class Problem
    {
        ...
        {
            sw.WriteLine("(:action invariant-R" + iNumber);
            ...
            if(Domain.ForceReasoningAfterObservation)
                sw.Write(" (ReasoningDone)");
            sw.WriteLine(")");
            sw.WriteLine(")");
        }
        ...
        public void WriteSingleReasoningAction(StreamWriter sw)
        {
            ...
            if (Domain.ForceReasoningAfterObservation)
            {
                sw.WriteLine("(:action ApplyReasoning");
                //sw.WriteLine(":precondition (and (ReasoningDone))"); //can run reasoning whenever, and multiple times.
                sw.WriteLine(":precondition (and (not-init))");
                sw.WriteLine(":effect (and (ReasoningDone) ");
            }
            ...
            {
                if (p.Value.Count == 1)
                    p.Key.Add(p.Value.First().Negate());
                ...
                foreach (Predicate pPrecondition in p.Key)
                {
                    cfPre.AddOperand(pPrecondition);
                }
                foreach (KnowPredicate pEffect in p.Value)
                {
                    cfEff.AddOperand(pEffect);
                }
                cfWhen.AddOperand(cfPre);
                cfWhen.AddOperand(cfEff);
                sw.WriteLine(cfWhen);
            }
            sw.WriteLine(")");
            sw.WriteLine(")");
        ...
        private void SimpleAddReasoningActions(CompoundFormula cf, HashSet<Predicate> lPredicates, Dictionary<HashSet<Predicate>, HashSet<Predicate>> dActions)
        {
            ...
            if (cf.Operator == "oneof")
            {
                if(cf.Operands.Count == 2)
                {
                    if (cf.Operands[0].Negate().Equals(cf.Operands[1]))
                        return;
                }
                foreach (Predicate p in lPredicates)
                {
                    ...
                    foreach (Predicate pOther in lPredicates)
                    {
                        if (pOther != p)
                        {
                            KnowPredicate kpOther = new KnowPredicate(pOther);
                            KnowPredicate knpOther = new KnowPredicate(pOther.Negate());

                            //Kp and p -> K everything else is false
                            lPreconditions = new HashSet<Predicate>();
                            lPreconditions.Add(kp);
                            lPreconditions.Add(kpOther.Negate());
                            lPreconditions.Add(knpOther.Negate());
                    ...
                    lPreconditions.Add(kp.Negate());
                    lPreconditions.Add(knp.Negate());
                    lEffects = new HashSet<Predicate>();
        ...
        public MemoryStream WriteKnowledgeProblem( HashSet<Predicate> lObserved)
        {
